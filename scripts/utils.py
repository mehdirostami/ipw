...
def simulate_x(n, p, rho, mean, sd, corr="AR(1)"):
...
def nonlinear(x, nonlinearity_portion=.30):
	...
	if p_nl == 0:
		...
		x_nls = [nlf[k](x[:, j], x[:, jj]).reshape(-1, 1)  for k, j, jj in zip(r, arg1, arg2)]

		x_nls = np.concatenate(x_nls, axis=1)
		x_nonlinear = np.hstack((x[:, p_nl:], x_nls))

		return(x_nonlinear)



def simulate_params(p1, r):
